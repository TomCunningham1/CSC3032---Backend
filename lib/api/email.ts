...
const handler = async (event: any): Promise<LambdaResponseType> => {
    ...
    if (!requestBody?.target || !requestBody?.score || !requestBody?.numberOfQuestions ||
        !requestBody?.numberOfAnsweredQuestions || !requestBody?.correctAnswers || !requestBody?.wrongAnswers || 
        !requestBody?.hintsUsed || !requestBody?.fiftyFiftyUsed) {
